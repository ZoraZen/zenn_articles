...
- `structlog`の布教記事を書くことで世界に平和を実現する。
- もうPythonのloggerモジュールの話をするの止めない？というお気持ち表明
- `structlog`の使い方に関する情報がネットにないので書く
- `structlog`の作者への寄付金が増えて欲しい、という祈り
...
# 構造化ロギングとは、ログの中にパースが難しく、一貫性を保ち難い平文を書くのではなく、
# キーと値のペアのコンテキストで起きたイベントをログに起こしていく手法です。
```
Pythonのloggerモジュール上でも構造化ログは実現できるが、structlogは最初から構造化ログに特化したAPIが用意されていることに特徴がある。

## `structlog`を使うモチベーション
### structlogの利点
以下は公式ドキュメントの冒頭とそれを翻訳したものです^[ドキュメント全体を俯瞰し曖昧な言葉が具体的になるよう意訳しました]。
```markdown
...
```
# 本題
logruと比較すると導入コストが高い、それがstructlogの欠点である。
logruはドキュメントのコードをコピペするだけで動くが、structlogにはそれだけで理解できるチュートリアルがなくドキュメントも難しい。^[チュートリアル、あるにはあるが、かなり上級者向き [Getting Started - structlog 23.1.0 documentation](https://www.structlog.org/en/stable/getting-started.html)]
...
上の画像の内容が簡単な使い方の例である。
これはstructlogを対話形式（REPL）から使用した結果で、`2023-06`から始まる内容が出力結果である。
- get_logger()でlogのオブジェクトを生成できる。それぞれのlogにはbindメソッドで属性を付与できる
- ログを出力する際に、文字列の他にもkeywordと値のペアを入力すれば、出力を構造化できる
- Pythonのloggingでは、深刻度に応じて出力をdebug、info、warning、error、criticalの間で使い分ける ^[[logging — Pythonドキュメント](https://docs.python.org/ja/3/library/logging.html#logging-levels)より]

## 私のstructlogテンプレート

上のチュートリアルではstructlogの初期設定をそのまま使用している。
structlogには設定の変更の余地が無限に残されている。また公式推奨のオススメ設定のようなものはない。
そこで設定の一例として私が使用しているテンプレートを共有する。
...
1. ターミナル上のlogの表示を綺麗にすること
2. logの結果を`aplication.log`にjson形式で保存すること
3. 情報を構造化して表示すること
...
まず、以下の`2つの目的`のために `handler_stdout` と `handler_file` をstructlog上で用意し、それをPythonのbuitinのloggerに渡している。
```markdown
1. ターミナル上のlogの表示を綺麗にすること
2. logの結果を`aplication.log`にjson形式で保存すること
```
`handler_stdout`には `ConsoleRenderer` を、`handler_file`には `JSONRenderer` をセットしています。
`ConsoleRenderer` はターミナル上でログを表示するための設定です。
ただ表示するのみならず、PythonのCUI表示を改善するライブラリである `colorama`^[Windows環境だと重要] と`better-exception`、`Rich`のパッケージがインストールされていれば、それらが使用され可読性が向上する。
これにより上述の`2つの目的`を達成している。

またデフォルト設定からの違いとして`structlog.stdlib.add_logger_name`の追加がある。
```diff python
processors=[
    ...
+   structlog.stdlib.add_logger_name,
...
以下はテンプレートを使用したプロダクションユースを模したコードである。
...
それぞれは実行時に`strategies`(あるいは`__main__`)、`StrategyB`、 `excecute`に評価される。
...
それを一人の開発者がOSSとしてメンテをしている。彼の凄さは筆舌に尽くしがたい。

structlogはproduction-readyを謳っており、普通の動作の範疇ではバグはほぼなく安定している。
私は実装を読みながらエッジケースで遊んでいたので、幾つかコミットを投げようと思ったが、修正には全体の仕様を深く理解するの必要があり、フルタイムのOSSメンテナーでないと厳しいように感じた。^[細かなPull Requestsは出したけど]
なので、コードで貢献できない私は寄付をした。^[この記事は寄付のステマです]
